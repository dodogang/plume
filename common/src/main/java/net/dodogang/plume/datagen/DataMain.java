...
import net.dodogang.plume.datagen.models.PlumeStateModelProvider;
import net.dodogang.plume.datagen.recipes.PlumeCraftingRecipeProvider;
import net.dodogang.plume.datagen.recipes.PlumeStonecuttingRecipeProvider;
import net.dodogang.plume.datagen.tags.PlumeBlockItemTagProvider;
...
public class DataMain {
    ...
    public static DataGenerator create(Path output, Collection<Path> inputs, boolean includeClient, boolean includeServer, boolean includeDev, boolean includeReports, boolean validate) {
        ...
        if (includeClient) {
            gen.install(new PlumeStateModelProvider(gen));
        }

        if (includeServer) {
//            gen.install(new PlumeFluidTagsProvider(gen));
//            PlumeBlockTagsProvider blockTags = new PlumeBlockTagsProvider(gen);
//            gen.install(blockTags);
//            gen.install(new PlumeItemTagsProvider(gen, blockTags));
            gen.install(new PlumeBlockItemTagProvider(gen));
            gen.install(new PlumeCraftingRecipeProvider(gen));
            gen.install(new PlumeStonecuttingRecipeProvider(gen));
            gen.install(new PlumeLootTableDataProvider(gen));
        }
